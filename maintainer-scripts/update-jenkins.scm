...
(define (generate-nix-exprs tags&revs)
  (let1 latest (rxmatch-substring (#/(?<=cdda-jenkins-)b\d+/ (caar tags&revs)))
...
(define (main _)
  ...
  (load-sha256-cache!)
  (let1 tags&revs (take (tree-map->alist (github-get-cdda-jenkins-tags))
                        *builds-to-keep*)
    (with-output-to-file (build-path "jenkins.nix")
